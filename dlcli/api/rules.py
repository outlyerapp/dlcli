...
class Rules(object):
    ...
    def import_rule(self, rule_path):
        ...
        requests.post(utils.build_api_url(self.ctx, 'rules'), headers=self.headers, data={"name": rule_name})
        self.headers.update({"Content-Type": "application/yaml"})
        requests.put(utils.build_api_url(self.ctx, 'rules' + '/' + rule_name), headers=self.headers, data=rule_content)
